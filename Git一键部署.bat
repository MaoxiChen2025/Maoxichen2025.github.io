...
:: 部署流程（核心修改：取消强制覆盖，保留本地文件）
echo ==============================================
echo 正在拉取远程仓库最新内容（保留本地修改）...
:: 移除 git reset --hard，改用普通拉取（冲突时提示，不覆盖）
git pull origin %github_branch% --no-rebase 2> nul
if errorlevel 1 (
    echo 提示：远程仓库为空/拉取无冲突，继续部署...
)
